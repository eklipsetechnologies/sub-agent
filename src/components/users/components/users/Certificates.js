...
UNSAFE_componentWillReceiveProps(nextProps){
    if (nextProps.data.quick_params !== this.props.data.quick_params) {
        if (nextProps.data.quick_param_name === 'CERT') {
            this.LoadData();
        }
    }
}
  ...
  this.LoadData()
    ...
    LoadData=()=>{
      ...
      if (localStorage.getItem('userToken')) {
          ...
          Axios.post(`${Home}auth/profile/listCertificates`,{
             token: token,
        ...
        if (this.props.data.quick_switch === 'WHOCODED_ED_EDIT') {
            ...
            <div className="card  border-0 mb-3">
                        <div className="card-body">
                            <h6 className="lh-5 mg-b-0">Certificates <button onClick={()=>this.QuickSwitcher('WHOCODED_ED_EDIT')} className="btn btn-sm btn-outline-primary  pull-right">Add more</button></h6>
                            {this.state.loading ?
                             <div className="p-4">
                                 <Spinner size={40} />
                             </div> 
                            ...
                            this.state.data.map((next,i)=>
                                <div onClick={()=>this.OpenModal('WHOCODED',i)} key={next.id} className="media mt-5">
                                 ...
                                 <div className="media-body pd-l-25">
                                    <h5 className="mg-b-5">{next.certificate} in {next.institution}</h5>
                                    <p className="mg-b-3"><span className="tx-medium tx-color-02">{next.course}</span>,  ({next.license})</p>
                                    <span className="d-block tx-13 tx-color-03">{next.date}</span>
                              </div>
                             </div>
                                )
                    }
...
export default connect(mapStoreToProps) (Certificates);