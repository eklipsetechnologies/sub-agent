...
class ListPayment extends Component {
    ...
    if (window.confirm("❌ are you sure you want to decline?")) {
      ...
      if (reason != null) {
        if (localStorage.getItem("userToken")) {
          ...
          Axios.post(`${Home}admin/declinePaymentInformation`, {
            id: key,
            reason:reason
          },{
              headers: { 
  ...
  Approve(key) {
    if (window.confirm("are you sure you want to approve?")) {
      ...
      if (localStorage.getItem("userToken")) {
        token = JSON.parse(localStorage.getItem("userToken"));
        this.setState({ loading: true });
        Axios.post(`${Home}admin/approvePaymentInformation`, {
          id: key,
        },{
            headers: { 
                'Authorization': `Bearer ${token}`
  ...
  LoadData = (filter) => {
    ...
    if (localStorage.getItem("userToken")) {
      token = JSON.parse(localStorage.getItem("userToken"));
      this.setState({ loading: true });
      Axios.get(`${Home}admin/getPaymentInformation`, {
        headers: { 
            'Authorization': `Bearer ${token}`
        } 
      })
            ...
            <div className="card-body">
              <div className="row">
                <div className="col-md-5">
                  <h6 class="lh-5 mg-b-0">Users Payment Information</h6>
                </div> 
...
export default connect(mapStoreToProps)(ListPayment);
