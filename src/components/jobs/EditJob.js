 ...
 Remove=(name)=>{
  ...
  LoadDataSingle = (filter) => {
    ...
    if (localStorage.getItem("userToken")) {
      token = JSON.parse(localStorage.getItem("userToken"));
      this.setState({ loading: true });
      Axios.get(`${Home}admin/geJobsId/${this.props.data.params.length > 0 ? this.props.data.params[0] : 0}`, {
        headers: { 
            'Authorization': `Bearer ${token}`
        } 
      })
        .then((res) => {
          ...
          if (res.data.payload.length > 0) {
            this.setState({
                premium:res.data.payload[0].title,
                steps:res.data.payload[0].amount,
                desc:res.data.payload[0].desc,
                number:JSON.stringify(res.data.payload[0].people),
                location:res.data.payload[0].location,
                lat:res.data.payload[0].lat,
                long:res.data.payload[0].long,
                address2:res.data.payload[0].address2,
                company:res.data.payload[0].category, 
                startDate:new Date(res.data.payload[0].start_date),
                startDate2:new Date(res.data.payload[0].end_date), 
            })
          }
        })
        .catch((err) => console.log(err));
          ...
          <div className="card border-0">
            <div className="card-body">
              ...
              <form onSubmit={this.handleSubmit} className="mt-4 row">
                ...
                <div className="col-md-6">
                  <div className="form-group ">
                    <label>Start Time ({moment(this.state.startDate).format("YYYY-MM-DD hh:mm:ss A")})</label>
                        <DatePicker 
                ...
                <div className="col-md-6">
                  <div className="form-group ">
                    <label>End  Time ({moment(this.state.startDate2).format("YYYY-MM-DD hh:mm:ss A")})</label>
                    <DatePicker 
                                        // maxDate={new Date('2020-12-31')}
...
export default connect(mapStoreToProps)(EditJob);
