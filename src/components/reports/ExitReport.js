...
class ExitReports extends Component {
    constructor(props) {
        super(props);
        this.state = {
    ...
    onChange2 = (startDate) =>{
      ...
      LoadData=()=>{
        ...
        if (localStorage.getItem('userToken')) {
            ...
            axios.get(`${Home}auth/reports/ExitReport`,{
              params:{token: token}
            })
           .then(res => {
           console.log(res);
    ...
    LoadData2=()=>{
        ...
        if (localStorage.getItem('userToken')) {
            token = JSON.parse(localStorage.getItem('userToken'));
            this.setState({loading:true});
            axios.get(`${Home}auth/reports/ExitReport2`,{
              params:{token: token}
            })
           .then(res => {
           console.log(',,.,.,,.',res);
...
export default connect(mapStateToProps) (ExitReports);