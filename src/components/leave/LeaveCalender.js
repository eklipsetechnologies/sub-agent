...
class LeaveCalender extends Component {
    ...
    LoadData2=(filter)=>{
        ...
        if (localStorage.getItem('userToken')) {
            token = JSON.parse(localStorage.getItem('userToken'));
            this.setState({loading:true});
            Axios.get(`${Home}auth/leave/listLeaveRequestCalender/${filter}`,{
             params:{token: token}
            })
           .then(res => {
            console.log(res);
           ...
           {/* <Toaster /> */}
            <BounceUp duration="1s" timingFunction="ease-out">
                ...
                <div className="row">
                    ...
                    <h6 class="lh-5 mg-b-0">Our Leave Calender</h6>
                    </div>
...
export default connect(mapStoreToProps) (LeaveCalender);