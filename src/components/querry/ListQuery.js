...
import {PlusCircle, Edit, Trash2, List, ArrowLeft, Eye, Mail, Command, Send, HardDrive, Flag } from 'react-feather';
...
class ListQuery extends Component {
    ...
    LoadData=(filter)=>{
        ...
        if (localStorage.getItem('userToken')) {
            token = JSON.parse(localStorage.getItem('userToken'));
            this.setState({loading:true});
            Axios.get(`${Home}auth/query/list/${filter}`,{
             params:{token: token}
            })
           .then(res => {
            console.log(res);
        ...
        return (
            ...
            <BounceRight duration="1s" timingFunction="ease-out">
                ...
                <div className="row">
                    ...
                    <h6 class="lh-5 mg-b-0">List of disciplinary querry</h6>
                    </div>
                    <div className="col-md-7">
                        <div className="pull-right">
                        
                        <FadeIn duration="1s" timingFunction="ease-out">
                        <button onClick={()=>this.SwitchContent('exit_add',[0])} className="btn btn-primary2 btn-sm shadow"><PlusCircle color="white" size={35} /> New Disciplinary Query</button>
                        ...
                        <tbody>
                            {this.state.data.map((dep,i)=>
                                <tr key={i}>
                                    ...
                                    <td>{dep.surname}</td>
                                    <td>{dep.subject.length >  80 ? dep.subject.slice(0,80)+'...':dep.subject}</td>
                                    <td>{dep.note.length >  40 ? dep.note.slice(0,40)+'...':dep.note}</td>
                                        ...
                                        <div>
                                            <FadeIn duration="1s" timingFunction="ease-out">
                                                ...
                                                :''}
                                                 ...
                                                 ''
                                                    // <button data-rh="View query" onClick={()=>this.OpenModal('WHOCODED',i)} className="btn btn-primary2 btn-sm m-1 shadow"><Eye color="white" size={35} /> </button>
                                                :''}
                                                ...
                                                :''}
                                                 <button data-rh="View query" onClick={()=>this.OpenModal('WHOCODED',i)} className="btn btn-primary2 btn-sm m-1 shadow"><Eye color="white" size={35} /> </button>
                                                 <button data-rh="Check responses" onClick={()=>this.SwitchContent('query_res',[dep.id])} className="btn btn-primary2 btn-sm m-1 shadow"><Mail color="white" size={35} /> </button>
                                                 <button data-rh="Query Action" onClick={()=>this.SwitchContent('query_action',[dep.id])} className="btn btn-warning btn-sm m-1 shadow"><Flag color="#000" size={35} /> </button>
...
export default connect(mapStoreToProps) (ListQuery);