 ...
 Remove=(name)=>{
  ...
  handleSubmit = (event) => {
    ...
    if (localStorage.getItem("userToken")) {
        ...
        Axios.post(`${Home}admin/editCredential`, {
  ...
  LoadDataSingle = (filter) => {
    ...
    if (localStorage.getItem("userToken")) {
      token = JSON.parse(localStorage.getItem("userToken"));
      this.setState({ loading: true });
      Axios.get(`${Home}admin/getSingleCredential/${this.props.data.params.length > 0 ? this.props.data.params[0] : 0}`, {
        headers: { 
            'Authorization': `Bearer ${token}`
        } 
      })
        .then((res) => {
          ...
          if (res.data.payload.length > 0) {
            this.setState({
                premium:res.data.payload[0].name,
                steps:res.data.payload[0].step,
                desc:res.data.payload[0].desc,
                seleted:res.data.payload[0].options?res.data.payload[0].options:[],
                company:res.data.payload[0].validation,
                markup:res.data.payload[0].option,
            })
          }
        })
        .catch((err) => console.log(err));
...
export default connect(mapStoreToProps)(EditCredential);
