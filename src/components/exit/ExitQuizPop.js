...
class ExitQuizPop extends Component {
    constructor(props) {
        super(props);
        this.state={
            style:"",
    ...
    LoadData=()=>{
     ...
     SubmitQuiz=()=>{
        ...
        if (localStorage.getItem('userToken')) {
            let token = "";
            token = JSON.parse(localStorage.getItem('userToken'));
            Axios.post(Home+`auth/exit/submitInterview`,{
                token:token,
                requestId:this.props.data.params.length > 0 ? this.props.data.params[0] : 0,
        ...
        return (
            <div>
                ...
                :
                    ...
                    <small className="text-muted text-center p-1">Click the button below to take your exit interview</small>
                        <div className="p-2">
...
export default connect(mapStateToProps) (ExitQuizPop);