...
import PopNextOfKin from './PopNextOfKin';

class NextOfKinRequest extends Component {
    constructor(props) {
        super(props);
        this.state = {
    ...
    LoadData=(filter)=>{
        ...
        if (localStorage.getItem('userToken')) {
            token = JSON.parse(localStorage.getItem('userToken'));
            this.setState({loading:true});
            Axios.get(`${Home}auth/profile/req/ReqlistNextOfKin/${filter}`,{
             params:{token: token}
            })
           .then(res => {
            console.log(res);
        ...
        return (
            ...
            <BounceRight duration="1s" timingFunction="ease-out">
                ...
                <div className="row">
                    ...
                    <h6 class="lh-5 mg-b-0">List of next of kin request</h6>
                    </div>
...
export default connect(mapStoreToProps) (NextOfKinRequest);