...
class PopCertificate extends Component {
    constructor(props) {
        super(props);
        this.state={
            style:"",
    ...
    handleSubmit=(event)=>{
     ...
     Approve=()=>{
        ...
        if (localStorage.getItem('userToken')) {
            token = JSON.parse(localStorage.getItem('userToken'));
            this.setState({laoding:true});
            Axios.post(`${Home}auth/profile/req/activateCertificates`,{
               token: token,
    ...
    Decline=()=>{
        ...
        if (localStorage.getItem('userToken')) {
            token = JSON.parse(localStorage.getItem('userToken'));
            this.setState({laoding:true});
            Axios.post(`${Home}auth/profile/req/declineCertificates`,{
               token: token,
               id:this.props.details.id
            })
           .then(res => {
            ...
            <div>
                ...
                <div className="modal-dialog " role="document">
                    ...
                    <div className="modal-header">
                        <h5 className="modal-title">View Certification Request</h5>
                        ...
                        :
                          <table className="table mt-2 table-hover table-bordered">
                              <tbody>
                                  ...
                                  <tr>
                                      <th>Course</th>
                                      <td>{this.props.details.course}</td>
                                  </tr>
                                  <tr>
                                      <th>Certificate</th>
                                      <td>{this.props.details.certificate}</td>
                                  </tr>
                                  <tr>
                                      <th>License</th>
                                      <td>{this.props.details.license}</td>
                                  </tr>
...
export default connect(mapStateToProps) (PopCertificate);