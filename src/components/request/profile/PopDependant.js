...
class PopDependant extends Component {
    constructor(props) {
        super(props);
        this.state={
            style:"",
    ...
    handleSubmit=(event)=>{
     ...
     Approve=()=>{
        ...
        if (localStorage.getItem('userToken')) {
            token = JSON.parse(localStorage.getItem('userToken'));
            this.setState({laoding:true});
            Axios.post(`${Home}auth/profile/req/activateDependant`,{
               token: token,
    ...
    Decline=()=>{
        ...
        if (localStorage.getItem('userToken')) {
            token = JSON.parse(localStorage.getItem('userToken'));
            this.setState({laoding:true});
            Axios.post(`${Home}auth/profile/req/declineDependant`,{
               token: token,
               id:this.props.details.id
            })
           .then(res => {
            ...
            <div>
                ...
                <div className="modal-dialog " role="document">
                    ...
                    <div className="modal-header">
                        <h5 className="modal-title">View Dependant Request</h5>
...
export default connect(mapStateToProps) (PopDependant);