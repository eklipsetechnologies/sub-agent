...
class TrainAllocation extends Component {
    constructor(props) {
        super(props);
        this.state = {
...
OpenModal2=(name,id)=>{
    ...
    LoadData=(filter)=>{
        ...
        if (localStorage.getItem('userToken')) {
            token = JSON.parse(localStorage.getItem('userToken'));
            this.setState({loading:true});
            Axios.get(`${Home}auth/train/listAllocation/${filter}`,{
             params:{token: token}
            })
           .then(res => {
            console.log(res);
        ...
        :''}
            ...
            <BounceRight duration="1s" timingFunction="ease-out">
                ...
                <div className="row">
                    ...
                    <h6 class="lh-5 mg-b-0">List of traing allocations</h6>
                    </div>
                    <div className="col-md-8">
                        ...
                        <button onClick={()=>this.SwitchContent('train_al_new',[0])} className="btn m-1 btn-primary2 btn-sm shadow"><PlusCircle color="white" size={35} /> New Training Allocation</button>
                        <DatePicker 
                        showYearPicker
                                            required
                                    ...
                                    {/* <td>
                                        <div>
                                            <FadeIn duration="1s" timingFunction="ease-out">
                                                ...
                                                <Link to={dep.letter} data-rh="Download Leave Prove" onClick={()=>this.OpenModal2('WHOCODED',dep.id)} className="btn btn-warning btn-sm m-1 shadow"><Download color="#000000" size={35} /></Link>
                                                ...
                                                :''}
                                                 {this.props.data.userDetails.hr  || this.props.data.userDetails.em ?
                                                    <button data-rh="View Request Actions" onClick={()=>this.OpenModal('WHOCODED',i)} className="btn btn-primary2 btn-sm m-1 shadow"><Command color="white" size={35} /> Request Actions </button>
                                                :''}
...
export default connect(mapStoreToProps) (TrainAllocation);