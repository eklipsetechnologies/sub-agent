 ...
 Remove=(name)=>{
  ...
  handleSubmit = (event) => {
    ...
    if (localStorage.getItem("userToken")) {
        ...
        Axios.post(`${Home}admin/editDocument`, {
  ...
  LoadDataSingle = (filter) => {
    ...
    if (localStorage.getItem("userToken")) {
      token = JSON.parse(localStorage.getItem("userToken"));
      this.setState({ loading: true });
      Axios.get(`${Home}admin/getSingleDocuments/${this.props.data.params.length > 0 ? this.props.data.params[0] : 0}`, {
        headers: { 
            'Authorization': `Bearer ${token}`
        } 
      })
...
export default connect(mapStoreToProps)(EditDocument);
