...
class ListDocFiles extends Component {
  constructor(props) {
    super(props);
    this.state = {
    ...
    if (window.confirm("❌ are you sure you want to decline?")) {
      ...
      if (reason != null) {
        if (localStorage.getItem("userToken")) {
          ...
          Axios.post(`${Home}admin/declineDocuments`, {
            id: key,
            reason:reason
          },{
              headers: { 
  ...
  LoadData = (filter) => {
    ...
    if (localStorage.getItem("userToken")) {
      ...
      Axios.get(`${Home}admin/getDocuments/${id}`, {
        headers: { 
            'Authorization': `Bearer ${token}`
        } 
      })
...
export default connect(mapStoreToProps)(ListDocFiles);
