...
import PolicyPaidById from '../components/policypaidbyid/PolicyPaidById';

const PolicyPaidBy = (props) => {
  useEffect(() => {
    // This code will run when the component mounts (equivalent to componentDidMount)
    props.dispatch(open_menu('PolicyPaidBy'));
    props.dispatch(open_main_menu('PolicyPaidBy'));
    props.dispatch(change_breadcrum('PolicyPaidBy'));
...
export default connect(mapStoreToProps)(PolicyPaidBy);
