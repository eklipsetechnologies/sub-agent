...
import ExitQContainer from '../components/settings/exit_questions/ExitQContainer';
import { submenu } from '../store/actions/SubMenu';

class SettingsExitQuestions extends Component {

    componentDidMount(){
        this.props.dispatch(open_menu('Control'));
        this.props.dispatch(open_main_menu('Control'));
        this.props.dispatch(change_breadcrum('Exit Questions'));
        this.props.dispatch(submenu('13'));
    }
    render() {
        return (
            <div>
...
export default connect(mapStoreToProps) (SettingsExitQuestions);